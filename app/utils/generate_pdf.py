...
def generate_medication_pdf(patient, doctor, appointment, medications):
    ...
    patient_folder = (
        f"pdfs/{patient.last_name}_{patient.first_name}_{patient.patronymic}".replace(
            " ", "_"
        )
    )
    ...
    c.drawString(
        50,
        height - 140,
        f"Пациент: {patient.last_name} {patient.first_name} {patient.patronymic}",
    )
    ...
    c.drawString(
        50,
        height - 260,
        f"Лечащий врач: {doctor.last_name} {doctor.first_name} {doctor.patronymic}",
    )
    ...
    if medications:
        for idx, med in enumerate(medications, 1):
            c.drawString(
                60, y, f"{idx}. {med.medication_name}, дозировка: {med.dosage}"
            )
            y -= 20
    else:
    ...
    c.drawString(
        50,
        y_footer - 40,
        f"Дата выдачи документа: {datetime.now().strftime('%d.%m.%Y')}",
    )
